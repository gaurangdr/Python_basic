...
boot_linux_tql() {
	...
	# Extract the test name before ".bif" and assign
		...
		echo "FROM \$OSL_DIR/zynqMP/hardware/scripts/execute/boot_linux/${bootmode}/dc1/${version}_bit" >> $tqlfile
		echo "TEMPLATE" >> $tqlfile
		echo -e "\t user { " >> $tqlfile
		echo -e "\t\t tsuite = '$bootmode'," >> $tqlfile
		echo -e "\t\t ssuite = 'boot_linux'," >> $tqlfile
		echo -e "\t\t version = '${version}_bit'," >> $tqlfile
		echo -e "\t\t module = 'dc1'," >> $tqlfile
...
boot_linux_secure_tql() {
	...
	# Extract the test name before ".bif" and assign
		...
		echo "FROM \$OSL_DIR/zynqMP/hardware/scripts/execute/boot_linux_secure/${bootmode}/dc1/${version}_bit" >> $tqlfile
		echo "TEMPLATE" >> $tqlfile
		echo -e "\t user { " >> $tqlfile
		echo -e "\t\t tsuite = '$bootmode'," >> $tqlfile
...
freertos_tql() {
	...
	# Extract the test name before ".bif" and assign
		...
		if echo "${biffile_name}" | grep -q "_all.bif"; then
			...
			export test_device="$DC1_PS"
		fi

		echo "FROM \$OSL_DIR/zynqMP/hardware/scripts/execute/freertos/${bootmode}/dc1/${version}_bit" >> $tqlfile
		echo "TEMPLATE" >> $tqlfile
		echo -e "\t user { " >> $tqlfile
		echo -e "\t\t tsuite = '$bootmode'," >> $tqlfile
...
linux_tql() {
	...
	echo "FROM \$OSL_DIR/zynqMP/hardware/scripts/execute/linux/dc1" >> $tqlfile
	echo "TEMPLATE" >> $tqlfile
	echo -e "\t user { " >> $tqlfile
...
uboot_tql() {
	...
	echo "FROM \$OSL_DIR/zynqMP/hardware/scripts/execute/uboot/dc1" >> $tqlfile
	echo "TEMPLATE" >> $tqlfile
	echo -e "\t user { " >> $tqlfile
...
zc1751_tql() {
	...
	echo "FROM \$OSL_DIR/zynqMP/hardware/scripts/execute/$ssuite/dc1" >> $tqlfile
	echo "TEMPLATE" >> $tqlfile
	echo -e "\t user { " >> $tqlfile
...
echo "######### Query for Boot Linux test cases for dc1 module ##############" >> $tqlfile
echo -e " " >> $tqlfile

echo "######## query for boot_linux_64 test cases #########" >> $tqlfile
echo -e " " >> $tqlfile

boot_linux_tql "osl" "weekly" "64" "qspi32" "$tqlfile"
boot_linux_tql "osl" "weekly" "64" "qspi24" "$tqlfile"
boot_linux_tql "osl" "weekly" "64" "sd1" "$tqlfile"
boot_linux_tql "osl" "sanity,daily" "64" "emmc" "$tqlfile"

#echo "######## query for boot_linux_32 test cases #########" >> $tqlfile
#echo -e " " >> $tqlfile
#
#boot_linux_tql "osl" "weekly" "32" "qspi32" "$tqlfile"
#boot_linux_tql "osl" "weekly" "32" "qspi24" "$tqlfile"
#boot_linux_tql "osl" "weekly" "32" "sd1" "$tqlfile"
#boot_linux_tql "osl" "sanity,daily" "32" "emmc" "$tqlfile"
...
tqlfile=boot_linux.tql
echo "######### Query for Boot Linux secure test cases for dc1 module ##############" >> $tqlfile
echo -e " " >> $tqlfile
...
boot_linux_secure_tql "osl" "sanity,daily" "64" "emmc" "$tqlfile"
...
#boot_linux_secure_tql "osl" "sanity,daily" "32" "emmc" "$tqlfile"
...
echo "######### Query for Freertos test cases for dc1 module ##############" >> $tqlfile
echo -e " " >> $tqlfile

echo "######## query for freertos_64 test cases #########" >> $tqlfile
echo -e " " >> $tqlfile

freertos_tql "weekly" "64" "qspi32" "$tqlfile"
freertos_tql "weekly" "64" "qspi24" "$tqlfile"
freertos_tql "weekly" "64" "sd1" "$tqlfile"
freertos_tql "sanity,daily" "64" "emmc" "$tqlfile"

#echo "######## query for freertos_32 test cases #########" >> $tqlfile
#echo -e " " >> $tqlfile
#
#freertos_tql "weekly" "32" "qspi32" "$tqlfile"
#freertos_tql "weekly" "32" "qspi24" "$tqlfile"
#freertos_tql "weekly" "32" "sd1" "$tqlfile"
#freertos_tql "sanity,daily" "32" "emmc" "$tqlfile"
...
generate_common_tql "$tqlfile" "\$OSL_DIR/zynqMP/hardware/scripts/execute/linux/dc1" "a53_linux" "linux" "dc1" "zc1751" "sanity,daily" "sanity,functional,performance,stress" "${COMMON_DIR}/msg.xml" "osl" "linux"
...
echo "######### Query for Open AMP test cases for dc1 module ##########" >> $tqlfile
echo -e " " >> $tqlfile
...
echo "######### Query for PMU Firmware test cases for dc1 module ##########" >> $tqlfile
echo -e " " >> $tqlfile

zc1751_tql "osl" "sanity,daily" "$tqlfile" "linux_cpu_hotplug" "pmu_fw_examples" "pmu_fw"

echo "######### Query for Xen test cases for dc1 module ##########" >> $tqlfile
echo -e " " >> $tqlfile
