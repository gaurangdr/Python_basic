...
boot_linux_tql() {
	...
	# Extract the test name before ".bif" and assign
		...
		echo "FROM \$OSL_DIR/zynqMP/hardware/scripts/execute/boot_linux/${bootmode}/dc2/${version}_bit" >> $tqlfile
		echo "TEMPLATE" >> $tqlfile
		echo -e "\t user { " >> $tqlfile
		echo -e "\t\t tsuite = '$bootmode'," >> $tqlfile
		echo -e "\t\t ssuite = 'boot_linux'," >> $tqlfile
		echo -e "\t\t version = '${version}_bit'," >> $tqlfile
		echo -e "\t\t module = 'dc2'," >> $tqlfile
...
boot_linux_secure_tql() {
	...
	# Extract the test name before ".bif" and assign
		...
		echo "FROM \$OSL_DIR/zynqMP/hardware/scripts/execute/boot_linux_secure/${bootmode}/dc2/${version}_bit" >> $tqlfile
		echo "TEMPLATE" >> $tqlfile
		echo -e "\t user { " >> $tqlfile
		echo -e "\t\t tsuite = '$bootmode'," >> $tqlfile
...
freertos_tql() {
	...
	# Extract the test name before ".bif" and assign
		...
		if echo "${biffile_name}" | grep -q "_all.bif"; then
			...
			export test_device="$DC2_PS"
		fi

		echo "FROM \$OSL_DIR/zynqMP/hardware/scripts/execute/freertos/${bootmode}/dc2/${version}_bit" >> $tqlfile
		echo "TEMPLATE" >> $tqlfile
		echo -e "\t user { " >> $tqlfile
		echo -e "\t\t tsuite = '$bootmode'," >> $tqlfile
...
linux_tql() {
	...
	echo "FROM \$OSL_DIR/zynqMP/hardware/scripts/execute/linux/dc2" >> $tqlfile
	echo "TEMPLATE" >> $tqlfile
	echo -e "\t user { " >> $tqlfile
...
uboot_tql() {
	...
	echo "FROM \$OSL_DIR/zynqMP/hardware/scripts/execute/uboot/dc2" >> $tqlfile
	echo "TEMPLATE" >> $tqlfile
	echo -e "\t user { " >> $tqlfile
...
zc1751_tql() {
	...
	echo "FROM \$OSL_DIR/zynqMP/hardware/scripts/execute/$ssuite/dc2" >> $tqlfile
	echo "TEMPLATE" >> $tqlfile
	echo -e "\t user { " >> $tqlfile
...
echo "######### Query for Boot Linux test cases for dc2 module ##############" >> $tqlfile
echo -e " " >> $tqlfile

echo "######## query for boot_linux_64 test cases #########" >> $tqlfile
echo -e " " >> $tqlfile

boot_linux_tql "osl" "sanity,daily" "64" "nand" "$tqlfile"

#echo "######## query for boot_linux_32 test cases #########" >> $tqlfile
#echo -e " " >> $tqlfile
#
#boot_linux_tql "osl" "sanity,daily" "32" "nand" "$tqlfile"
...
tqlfile=boot_linux.tql
echo "######### Query for Boot Linux secure test cases for dc2 module ##############" >> $tqlfile
echo -e " " >> $tqlfile

echo "######## query for boot_linux_secure_64 test cases #########" >> $tqlfile
echo -e " " >> $tqlfile

boot_linux_secure_tql "osl" "sanity,daily" "64" "nand" "$tqlfile"

#echo "######## query for boot_linux_secure_32 test cases #########" >> $tqlfile
#echo -e " " >> $tqlfile
#
#boot_linux_secure_tql "osl" "sanity,daily" "32" "nand" "$tqlfile"
...
echo "######### Query for Freertos test cases for dc2 module ##############" >> $tqlfile
echo -e " " >> $tqlfile

echo "######## query for freertos_64 test cases #########" >> $tqlfile
echo -e " " >> $tqlfile

freertos_tql "sanity,daily" "64" "nand" "$tqlfile"

#echo "######## query for freertos_32 test cases #########" >> $tqlfile
#echo -e " " >> $tqlfile
#
#freertos_tql "sanity,daily" "32" "nand" "$tqlfile"
...
#linux_tql "osl" "sanity,daily" "$tqlfile" "can"
linux_tql "osl" "sanity,daily" "$tqlfile" "smp"
linux_tql "osl" "sanity,daily" "$tqlfile" "nfs"
#linux_tql "osl" "sanity,daily" "$tqlfile" "spi"
#linux_tql "osl" "sanity,daily" "$tqlfile" "nand"
...
linux_tql "osl" "weekly" "$tqlfile" "nand_mtd_speedtest" "perf_test"
linux_tql "osl" "weekly" "$tqlfile" "nand_mtd_readtest" "perf_test"
linux_tql "osl" "weekly" "$tqlfile" "nand_mtd_oobtest" "perf_test"
linux_tql "osl" "weekly" "$tqlfile" "nand_mtd_stresstest" "perf_test"
linux_tql "osl" "weekly" "$tqlfile" "nand_fio" "perf_test"

generate_common_tql "$tqlfile" "\$OSL_DIR/zynqMP/hardware/scripts/execute/linux/dc2" "a53_linux" "linux" "dc2" "zc1751" "sanity,daily" "sanity,functional,performance,stress" "${COMMON_DIR}/msg.xml" "osl" "linux"
...
uboot_tql "osl" "sanity,daily" "$tqlfile" "nand_read"
uboot_tql "osl" "sanity,daily" "$tqlfile" "nand_write"
...
echo "######### Query for Open AMP test cases for dc2 module ##########" >> $tqlfile
echo -e " " >> $tqlfile

zc1751_tql "osl" "sanity,daily" "$tqlfile" "echo_test" "openamp_example_apps" "openamp"
zc1751_tql "osl" "sanity,daily" "$tqlfile" "mat_mul_demo" "openamp_example_apps" "openamp"
zc1751_tql "osl" "sanity,daily" "$tqlfile" "proxy_app" "openamp_example_apps" "openamp"

echo "######### Query for PMU Firmware test cases for dc2 module ##########" >> $tqlfile
echo -e " " >> $tqlfile

zc1751_tql "osl" "sanity,daily" "$tqlfile" "linux_cpu_hotplug" "pmu_fw_examples" "pmu_fw"

echo "######### Query for Xen test cases for dc2 module ##########" >> $tqlfile
echo -e " " >> $tqlfile

zc1751_tql "osl" "sanity,daily" "$tqlfile" "xen" "xen_hw" "xen"
