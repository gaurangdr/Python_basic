...
build_lnxuboot_tql() {
	...
	testcase_list_lnxuboot=(linux uboot)
	for testcase in "${testcase_list_lnxuboot[@]}"; do
		echo "FROM \$RDI_DEMO_DIR/build_tests/opensource/${testcase}" >> $tqlfile
		echo "TEMPLATE" >> $tqlfile
		echo -e "\tuser {" >> $tqlfile
		echo -e "\t\tssuite = 'ZYNQ_`echo ${testcase} | awk '{print toupper($0)}'`'," >> $tqlfile
		echo -e "\t\ttsuite = 'BUILD_`echo ${testcase} | awk '{print toupper($0)}'`'," >> $tqlfile
		fetch_name  ${ZYNQ_BUILD_TESTS}/build_tests/opensource/${testcase}
		echo -e "\t\tname = '${name}'," >> $tqlfile
		echo -e "\t\tmodule = '${testcase}'," >> $tqlfile
		fetch_cmd  ${ZYNQ_BUILD_TESTS}/build_tests/opensource/${testcase}
		if [ -n "$cmd" ]; then
		echo -e "\t\tcommand = '${cmd}'," >> $tqlfile
		fi
		echo -e "\t\tregression_type = 'sanity'," >> $tqlfile
		echo -e "\t\tpriority = '1'," >> $tqlfile
		echo -e "\t\tmessagefile = '${BUILD_DIR}/opensource/${testcase}/msg.xml'" >> $tqlfile
		echo -e "\t}" >> $tqlfile
		echo "AS build_zynq_projects_${testcase};" >> $tqlfile
		echo -e "" >> $tqlfile
	done
}
...
build_fsbl_tql() {
	...
	for testcase in "${testcase_list_fsbl[@]}"; do
		echo "FROM \$RDI_DEMO_DIR/build_tests/fsbl" >> $tqlfile
		echo "TEMPLATE" >> $tqlfile
		echo -e "\tuser {" >> $tqlfile
